...
\begin{multicols*}{3}
  ...
  \mbox{}\vspace{0.2em}
  Soit $\lim\limits_{n \to+\infty } \Big|\frac{a_{n+1}}{a_n}  \Big| = L$ \textbf{alors} si :   
  \begin{itemize}
    \item [$\rhd$ ] $L = 1 \implies$ \textit{inconclusif}  
    \item [$\blacktriangleright$ ] $L > 1 \implies \sum_{n=1}^{\infty }a_n$ \textbf{div}.   
  ...
  \mbox{}\vspace{0.2em}
  Soit $\lim\limits_{n \to+\infty } \sqrt[n]{\big| a_n \big|} = L$ \textbf{alors} si :   
  \begin{itemize}
    \item [$\rhd$ ] $L = 1 \implies$ \textit{inconclusif}  
    \item [$\blacktriangleright$ ] $L > 1 \implies \sum_{n=1}^{\infty }a_n$ \textbf{div}.   
    \item [$\blacktriangleright$ ] $L < 1 \implies \sum_{n=1}^{\infty }a_n$ \textbf{conv}.
...
\scalebox{1.2}{
  ...
  .4 $\vdots$.
  .5 f(j+1, n).
  .6 f(j, n) $\rightarrow$ 0 (cas de base).
  .6 f(j+1, n-1).
  .7 $\vdots$.
  .8 f(j+1, k+1).
  .9 f(j, k+1) $\rightarrow$ 0 (cas de base).
  .9 f(j+1, k) $\rightarrow$ ? (cas de base).
  .8 f(j+1, k) $\rightarrow$ ? (cas de base).
  .3 f(m-1, n-1).
  .4 $\vdots$.
  .2 f(m, n-1).
  .3 f(m-1, n-1).
  .4 $\vdots$.
  .5 f(j+1, n-1).
  .6 f(j, n-1) $\rightarrow$ 0 (cas de base).
  .6 f(j+1, n-2).
  .7 $\vdots$.
  .8 f(j+1, k+1).
  .9 f(j, k+1) $\rightarrow$ 0 (cas de base).
